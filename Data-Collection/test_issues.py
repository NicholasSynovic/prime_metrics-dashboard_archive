...
class Test_Issues(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.dbConnector.executeSQL(sql='CREATE TABLE Issues (ID INTEGER, Count INTEGER, Title TEXT, Author TEXT, Assignees TEXT, Labels TEXT, Created_At TEXT, Updated_At TEXT, Closed_At TEXT, PRIMARY KEY(ID));')
        
        #Test data
        cls.testData = {0: {'id': 0, 'number': 7357,'title': 'testTitle', 
                        'user': {'login': 'testAuthLogin'},
                        'assignees': {'login': 'A1Log', 'login': 'A2log'}, 
                        'labels': {'name': 'testLabel1', 'name': 'testLabel2'}, 
                        'state': 'testState', 'created_at': 'testCreate',
