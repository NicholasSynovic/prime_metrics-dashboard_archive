...
class Test_Forks(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.dbConnector.executeSQL(sql='CREATE TABLE Forks (ID TEXT, Name TEXT, Owner TEXT, Created_At TEXT, Updated_At TEXT, Pushed_At TEXT, Size INTEGER, Forks INTEGER, Open_Issues INTEGER, PRIMARY KEY(ID));')
        
        #Test data
        cls.testData = {0: {'id': "testID", 'name': 'testName',
                        'owner': {'login': 'testLogin'}, 'created_at': 'testCreate',
                        'updated_at': 'testUpdate', 'pushed_at': 'testPush',
                        'size': 7357, 'forks_count': 7357,
                        'open_issues_count': 7357}}
    
        cls.dataComparison = [('testID', 'testName', 'testLogin', 'testPrivate', 
                               'testFork', 'testCreate', 'testUpdate', 'testPush', 
                               7357, 7357, 7357)]

        #Commit class setup
        cls.forks = Forks(dbConnection = cls.dbConnector,
                             oauthToken = "token",
                             repository = "repo",
                             username = "name",
                             url = "url")
