...
sample_data_dict = {
    ...
    "commit": {
        "author": {
            "name": "Steph99rod",
            "email": "steph99rodriguez@gmail.com",
            "date": "2016-08-12T15:23:48Z",
        },
        "committer": {
            "name": "GitHub",
        ...
        "tree": {
            "sha": "86ad0e52c078ba17cf0938a3cd84dc8ed7762515",
...
class TestCommits(object):
    ...
    def test_get_github_data(self):
        assert (
            sample_commit.get_github_data(sample_commit.get_author_name, 0)
            == "Steph99rod"
        )
        assert (
            sample_commit.get_github_data(sample_commit.get_committer_name, 0)
            == "GitHub"
        )
        assert (
            sample_commit.get_github_data(sample_commit.get_message, 0)
            == "Add files via upload"
        )
        assert sample_commit.get_github_data(sample_commit.get_comment_count, 0) == 0
        assert (
            sample_commit.get_github_data(sample_commit.get_commits_url, 0)
            == "https://api.github.com/repos/Steph99rod/Overpriced-CollegeT/git/commits/691dd7efce02ea932803ea2aea248af62aa98bc8"
        )
        assert (
            sample_commit.get_github_data(sample_commit.get_comments_url, 0)
            == "https://api.github.com/repos/Steph99rod/Overpriced-CollegeT/commits/691dd7efce02ea932803ea2aea248af62aa98bc8/comments"
        )
        assert sample_commit.get_github_data(
            sample_commit.get_author_date, 0
        ) == datetime(2016, 8, 12, 15, 23, 48)
        assert sample_commit.get_github_data(
            sample_commit.get_committer_date, 0
        ) == datetime(2016, 8, 12, 15, 23, 48)
