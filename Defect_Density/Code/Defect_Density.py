...
def Main(c, conn, days):
    ...
    for day in days:
        ...
        Num_Of_Closed_BF = (
            ""  # Fill this with the total number of closed BF on that date
        )
        Num_Of_Closed_FR = (
            ""  # Fill this with the total number of closed FR on that date
        )
        ...
        c.execute(
            "SELECT COUNT(*) FROM ISSUES WHERE date(updated_at) <= date('"
            + str(day)
            + "');"
        )
        ...
        c.execute(
            "select total_lines from LINES_OF_CODE_NUM_OF_CHARS where date(date) = (select max(date(date)) from LINES_OF_CODE_NUM_OF_CHARS where date(date) <= date('"
            + str(day)
            + "'));"
        )
        try:
