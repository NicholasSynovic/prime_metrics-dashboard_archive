...
docker build . -t code
docker run -v $VOLUMENAME:/dataCollected code -u $1 -t $2 -o $3 

CONTAINERID="$(docker ps -q -n 1)"

# Copy volume data to the CWD
docker container cp $CONTAINERID:/dataCollected $cwd
...
echo "Data collection created and stored in ${cwd}/dataCollected"
